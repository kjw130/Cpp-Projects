...
namespace Utils{
    ...
    void PrintVector(std::vector<std::string> VectorToPrint, std::string Text){
        for(float i = 0; i<VectorToPrint.size(); i++){
            if(i == 0){
                std::cout << Text;
            }
...
class App{
    ...
    std::vector<std::string> AvailableCurrenciesToPrint = {"AUD", "USD", "GBP"};
    C_USD USD;
    C_AUD AUD;
    C_GBP GBP;
    ...
    public:
        void RunApp(){
            while(true){
                ...
                Utils::PrintVector(AvailableCurrenciesToPrint, "The currencies avaliable are: ");
